...
基于 Electron 的**开源桌面客户端**，对接 [HTW 媒体平台](https://htwmedia.dpdns.org) V2 API，把「选题、转写字幕、媒资洞察、内容创作、智能剪辑、多平台发布」全部收进一个工作台。
...
1. **🤖 Agent 原生**：全部能力同时以 Agent Skills 开放，AI 编程助手（Claude Code / Cursor 等）可直接编排「洞察 → 创作 → 剪辑 → 发布」全流程；
2. **🔗 全链路一站式**：媒资洞察、AI 创作、智能剪辑、多平台分发在一个工作台内闭环——多数竞品只做单点；
3. **🖥 三端同权**：Windows 桌面端 / Web 免安装端 / CLI 共用同一账号与全部能力，Mac 用户零门槛。
...
- [x] **选题雷达 Topics**（新）：
  - [x] **今日选题** —— 结合热榜趋势与你的垂类画像，每天生成带**数据证据**的选题卡（对标爆款倍数 / 热榜上升期 / 垂类缺口），一键进入创作
  - [x] **对标雷达** —— 添加 3 个对标账号，定时采样数据建基线；同行爆款（超基线 3 倍）自动提醒
  - [x] **热榜趋势** —— 小时级采样热度曲线，标注上升期 / 平台期 / 已过气，不再追错热点
  - [x] 接口契约与数据模型见 [docs/prd-topic-selection.md](docs/prd-topic-selection.md)
- [x] **媒资洞察**：
...
- [x] **内容创作**：AI 辅助一站式生成短视频脚本、图文与文章
  ...
  - [x] **短剧创作 ShortDrama**：描述剧情创意，由服务端智能创作技能自动完成规划、剧本、分镜与成片，支持多轮细化、分镜生成与剧本查看
...
桌面客户端目前主要提供 Windows 安装包，macOS 版为未签名构建、需自行编译。如果你在 Mac 或任意带浏览器的设备上使用，直接访问 **HTW 媒体平台 Web 端**即可获得与桌面端一致的完整能力：

- 地址：<https://htwmedia.dpdns.org>
- 功能：音频转写 / 字幕、媒资洞察、内容创作（视频 / 图片 / 文章）、智能剪辑（粗剪 / 草稿导出 / 超分 / 解密）、多平台发布（抖音 / 小红书 / B站 / 今日头条）等。
- 账号：在 Web 端「设置」中创建 `AuthKey`，桌面端与 Web 端共用同一账号与配额；其他设备粘贴该 Key 即可使用。

> 桌面端与 Web 端均调用同一套 `/api/v2/*` 契约，全部重活（转写、剪辑、渲染、发布）都在服务端完成，因此浏览器中的体验与客户端一致，无需安装即可在 Mac 上使用全部功能。
...
创作素材与平台账号是创作者的命根子，我们把数据策略摊开说（与代码行为一致，欢迎审阅 `apps/desktop/` 源码验证；策略如有变更会同步更新本表）：
