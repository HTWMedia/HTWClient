...
创作素材与平台账号是创作者的命根子，我们把数据策略摊开说（客户端处理逻辑可直接审阅 `apps/desktop/` 源码验证）：

| 问题 | 现状 |
| ---- | ---- |
| 素材 / 成片存在哪？ | 由服务端（HTW 媒体平台）处理与暂存 <!-- TODO: 补充保留时长与清理策略 --> |
| 平台 Cookie 会传给服务器吗？ | 发布时用于携带登录态调用平台接口 <!-- TODO: 确认 Cookie 仅本地拼接请求头，还是会存储到服务端，并如实修改此行 --> |
| 有人看我的内容吗？ | 转写 / 剪辑 / 发布均由自动化流水线处理，无人工介入 <!-- TODO: 确认 --> |
